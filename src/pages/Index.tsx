...
export default function Index() {
  ...
  async function handleAnalysisComplete() {
    // Attempt to fetch real tweets — falls back to demo mode silently if unavailable
    try {
      const tweets = await fetchLatestTweets(username);
      console.log(`✅ Fetched ${tweets.length} tweets for @${username}:`, tweets);
    } catch (e) {
      console.warn(`⚠️ X API unavailable for @${username} — running in demo mode:`, e);
    }
  ...
  const handleCopyImage = useCallback(async () => {
    ...
    try {
      // Pure canvas render — no html2canvas, no DOM capture, always correct output
  ...
  return (
    ...
    >
      ...
      <header className="relative z-10 flex items-center px-8 py-6">
        <svg height="28" viewBox="0 0 203 35" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="lunar strategy">
        ...
        <AnimatePresence mode="wait">
          ...
          {stage === "landing" && (
            ...
            >
              ...
              >
                discover your<br /><span className="whitespace-nowrap">Marketeer persona</span>
              </motion.h1>
              ...
              >
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full rounded-full pl-11 pr-4 py-4 text-sm focus:outline-none"
                    style={{
                      background: "#cde2f5",
                      border: "none",
                      color: BRAND,
                ...
                <button
                  type="submit"
                  className="px-8 py-4 rounded-full text-sm font-bold tracking-wide active:scale-[0.98] whitespace-nowrap"
                  style={{
                    background: BRAND,
                    color: "#ffffff",
                    fontFamily: "'Neue Haas Unica', sans-serif",
          ...
          {stage === "result" && persona && (
            ...
            >
              ...
              >
                <button
                  onClick={handleShare}
                  className="flex-1 flex items-center justify-center gap-2 px-5 py-3.5 rounded-full text-sm font-bold tracking-wide active:scale-[0.98]"
                  style={{
                    background: BRAND,
                    color: "#ffffff",
                    fontFamily: "'Neue Haas Unica', sans-serif",
