...
const CARD_BG = "#1800ad";
const ASCII_RAMP = '@#S08Xox+=;:-,. ';
const CELL_W = 11, CELL_H = 14;
const BG_R = 24, BG_G = 0, BG_B = 173;
const BASE_R = 80, BASE_G = 90, BASE_B = 210;

function drawOrb(canvas: HTMLCanvasElement) {
  ...
  for (let row = 0; row < H; row++) {
    for (let col = 0; col < W; col++) {
      ...
      const nxc = Math.max(-1, Math.min(1, dpx / RADIUS_PX)), nyc = Math.max(-1, Math.min(1, dpy / RADIUS_PX));
      ...
      ctx.globalAlpha = (edgeBlend * 0.92 + 0.08) * 0.45;
      if (lum > 0.6) { ctx.shadowBlur = lum * 10; ctx.shadowColor = 'rgba(150,160,255,0.7)'; }
...
const metrics = [
  { label: "HEARTS STOLEN",  value: "1"   },
  { label: "SMILES / DAY",   value: "99"  },
  { label: "CUDDLES / DAY",  value: "87"  },
  { label: "LOVE LEVEL",     value: "100" },
  { label: "BIG FISH",       value: "100" },
];

export default function PrincessCard() {
  ...
  return (
    ...
    >
      ...
      {/* ── Hero row ── */}
      <div
        className="relative z-10 flex items-center justify-between px-8 flex-1 gap-6"
      ...
      >
        ...
        <div className="flex flex-col justify-center" style={{ flex: "0 0 52%" }}>
          ...
          <div
            ...
            style={{ fontSize: "13px", color: "#FFE500", fontFamily: "'Neue Haas Unica', 'Helvetica Neue', Helvetica, Arial, sans-serif", fontWeight: 700 }}
          >
            👑 The Princess
          </div>
          <div
            className="mt-1 leading-relaxed"
            style={{ fontSize: "10px", color: "rgba(255,255,255,0.88)", fontFamily: "'Neue Haas Unica', 'Helvetica Neue', Helvetica, Arial, sans-serif", maxWidth: "90%", wordBreak: "break-word" }}
          >
            Demands perfection, manifests chaos, somehow always right. Science cannot explain it.
          </div>
        </div>
          ...
          {metrics.map(({ label, value }) => (
            <div key={label} className="flex items-baseline justify-between">
              <span
                ...
                style={{ fontSize: "10px", color: "rgba(255,255,255,0.85)", fontFamily: "'Neue Haas Unica', 'Helvetica Neue', Helvetica, Arial, sans-serif" }}
              >
                {label}
              </span>
              <span
      ...
      {/* Divider */}
      <div className="relative z-10 mx-8" style={{ height: "1px", background: "rgba(255,255,255,0.15)" }} />

      {/* ── Bottom bar ── */}
      <div className="relative z-10 flex items-end justify-between px-8 py-5">
        <div>
          <div
            className="tracking-widest uppercase mb-1"
            style={{ fontSize: "9px", color: "rgba(255,255,255,0.75)", fontFamily: "'Neue Haas Unica', 'Helvetica Neue', Helvetica, Arial, sans-serif" }}
          >
            DRAMA LEVEL
          </div>
          <div className="flex items-center gap-2">
            ...
            <div
              ...
              style={{ width: 52, height: 3, background: "rgba(255,255,255,0.18)" }}
            >
              <div
                className="absolute left-0 top-0 h-full rounded-full"
                style={{ width: "100%", background: "#FFE500" }}
              />
            </div>
          </div>
        </div>
        ...
        <div
          ...
          style={{ fontSize: "11px", color: "#FFE500", fontFamily: "'Neue Haas Unica', 'Helvetica Neue', Helvetica, Arial, sans-serif", fontWeight: 700 }}
        >
          isabell ♛
        </div>
...
export function PrincessCardAnimated() {
  return (
    <motion.div
